...
dpUrl=$(curl -sk "$orgUrl/availability-zones/availability-zone-default/gateway-services" \
 ...
 --data-binary "{\"name\":\"remoteapigateway\",\"title\":\"Remote API Gateway\",\"endpoint\":\"https://$ep_remote_gwd\",\"api_endpoint_base\":\"https://$ep_remote_gw\",\"tls_client_profile_url\":\"$tlsClientDefault\",\"gateway_service_type\":\"$ep_gwType\",\"visibility\":{\"type\":\"public\"},\"sni\":[{\"host\":\"*\",\"tls_server_profile_url\":\"$tlsServer\"}],\"integration_url\":\"$integration_url\"}" \
 ...
 curl -sk -X PATCH \
   "$orgUrl/availability-zones/availability-zone-default/gateway-services/remoteapigateway" \
 -H 'Accept: application/json' \
 -H "Authorization: Bearer $access_token"\
